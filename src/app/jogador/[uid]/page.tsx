...
function Visit(){
 ...
 useEffect(()=>{if(!uid)return;(async()=>{const [p,c]=await Promise.all([getDoc(doc(db,"publicProfiles",uid)),getDoc(doc(db,"cafes",uid))]);setData({profile:p.data(),cafe:c.data()});try{await httpsCallable(functions,"recordVisit")({targetUid:uid})}catch{}})()},[uid]);
