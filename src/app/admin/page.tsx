...
function Admin(){
 ...
 async function grantCoins(uid:string,amount:number){try{await httpsCallable(functions,"adminGrant")({targetUid:uid,type:"coins",amount});setMsg(`✅ ${amount.toLocaleString("pt-BR")} moedas enviadas.`)}catch(e:any){setMsg(`⚠️ ${e.message}`)}}
 async function grantGift(uid:string,giftId:string){try{await httpsCallable(functions,"adminGrant")({targetUid:uid,type:"gift",giftId});setMsg(`🎁 Presente ${giftId} enviado.`)}catch(e:any){setMsg(`⚠️ ${e.message}`)}}
